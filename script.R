...
fn <- function(x){
  ...
  out$wave <- which(LETTERS %in% x$WAVE)
  out <- out %>% select(hid, wave, pid, pno, opno, opid,
                        relation = rel,
...
fn <- function(x){
  ...
  out$wave <- which(LETTERS %in% x$WAVE)
  out <- out %>% select(hid, wave, hstype, simplehstype, region, tenure, simpletenure,
...
x13_nodrive <- setx(
  z13,
#  fn = list(wave = 1, ego_sex = "male", alter_sex = "female", ego_age = 25)
