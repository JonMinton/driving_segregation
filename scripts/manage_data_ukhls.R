...
search_patterns <- c(
  ...
  "^[a-z]{1}_hhorig$", # Sample origin - BHPS are 3, 4, 5, 6
...
all_inds_drvs %>% 
  select(
    pid, wave, hidp, sex, age, dlo, cu
  ) -> all_inds_drvs

write_csv(all_inds_drvs, path = "data/derived/ukhls_osms_drivers_simplified.csv")
